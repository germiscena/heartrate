export function detectRPeaksATN(signalData, filteredSignal) {
  ...
  const winEnergyMs = 80;
  const alignWinMs = 20;
  const minHeight = 0;
  const minPromMs = 60;
  const minProm = 0;

  const absVec = (v) => v.map(Math.abs);

  function movingAverageCentered(v, w) {
  ...
  function windowMin(v, a, b) {
    let m = Infinity;
    for (let i = a; i <= b; i++) if (v[i] < m) m = v[i];
    return m;
  }

  function argmax(v, a, b) {
    let bestI = a,
      bestV = v[a];
    for (let i = a; i <= b; i++)
      if (v[i] > bestV) {
  ...
  const e = new Array(N);
  const a1 = absVec(filteredSignal1),
    a2 = absVec(filteredSignal2),
    a3 = absVec(filteredSignal3),
    a4 = absVec(filteredSignal4);
  for (let i = 0; i < N; i++) e[i] = a1[i] + a2[i] + a3[i] + a4[i];

  const winE = Math.max(1, Math.round((winEnergyMs / 1000) * fs));
  const E = movingAverageCentered(e, winE);

  const events = [];
  const promW = Math.max(1, Math.round((minPromMs / 1000) * fs));
  for (let i = 1; i < N - 1; i++) {
    if (
      signalData[i] > 0 &&
      signalData[i] >= minHeight &&
      signalData[i] > signalData[i - 1] &&
      signalData[i] >= signalData[i + 1]
    ) {
      const aL = Math.max(0, i - promW),
        bL = i - 1;
      const aR = i + 1,
        bR = Math.min(N - 1, i + promW);
      const leftMin = bL >= aL ? windowMin(signalData, aL, bL) : signalData[i] - minProm;
      const rightMin = bR >= aR ? windowMin(signalData, aR, bR) : signalData[i] - minProm;
      const promOK = signalData[i] - leftMin >= minProm && signalData[i] - rightMin >= minProm;
      if (promOK) events.push(i);
    }
  }
  if (events.length === 0) return [];

  const meanE = events.reduce((s, p) => s + E[p], 0) / events.length;
  let NL = meanE * 0.9;
  let SL = meanE * 1.1;

  const alignW = Math.max(1, Math.round((alignWinMs / 1000) * fs));
  const accepted = [];
  ...
  for (let k = 0; k < events.length; k++) {
    ...
    const a = Math.max(0, i0 - alignW),
      b = Math.min(N - 1, i0 + alignW);
    const i = argmax(E, a, b);

    const Px = E[i];
    let Ds;
    if (Px <= NL) Ds = 0;
    else if (Px >= SL) Ds = 1;
    else Ds = (Px - NL) / Math.max(1e-12, SL - NL);

    if (Ds > tau && signalData[i] > 0) {
      if (accepted.length > 0 && i - accepted[accepted.length - 1] < refr) {
        const last = accepted[accepted.length - 1];
        if (signalData[i] > signalData[last]) accepted[accepted.length - 1] = i;
        SL = (1 - lambda) * Math.max(Px, E[accepted[accepted.length - 1]]) + lambda * SL;
      } else {
        accepted.push(i);
        SL = (1 - lambda) * Px + lambda * SL;
      }
    } else {
      NL = (1 - lambda) * Px + lambda * NL;
    }
  }
