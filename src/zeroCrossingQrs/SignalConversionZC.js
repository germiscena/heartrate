...
const ind_pos = [];
difference_pos.forEach((diff, i) => { if (diff > 1) ind_pos.push(i); });

const ind_neg = [];
difference_neg.forEach((diff, i) => { if (diff > 1) ind_neg.push(i); });
...
for (let j = 0; j < final_event.length; j++) {
  const [start, end] = final_event[j];
  const segment = nonlinear.slice(start, end + 1);
  abs_max.push(Math.max(...segment.map(Math.abs)));
  abs_min.push(Math.min(...segment.map(Math.abs)));
}
...
return {finalPeaks:RR_time = RR_time.filter(v => v !== null),d,D}
